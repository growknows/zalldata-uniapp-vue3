export default function get_enableVue3MpClick(sensors) {
  ...
  return function enableVue3MpClick() {
    ...
    function platformProxy (sfcComponent) {
      let oldRender = sfcComponent.render || sfcComponent.setup;
      ...
      const _render = function (ctx) {
        var bindingConfig = oldRender.apply(sfcComponent, arguments);
      ...
      if(sfcComponent.render) {
        sfcComponent.render = _render
      }else if(sfcComponent.setup) {
        sfcComponent.setup = _setup
      }
    ...
    if (oldCreatePage) {
      platform.createPage = platformProxy
    }

    let oldCreateComponent = platform.createComponent;
    if (oldCreateComponent) {
      platform.createComponent = platformProxy
    }
