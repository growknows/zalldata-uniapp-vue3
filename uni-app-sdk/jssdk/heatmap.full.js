        ...
        return function (elem) {
          if ("form" in elem) {
            if (elem.parentNode && elem.disabled === false) {
              ...
              return (
                elem.isDisabled === disabled || (elem.isDisabled !== !disabled && disabledAncestor(elem) === disabled)
              );
            }
                ...
                return function () {
                  var that = this,
                    ...
                    mightThrow = function () {
                      ...
                      then =
                        returned && (typeof returned === "object" || typeof returned === "function") && returned.then;
    ...
    jQuery.Event = function (src, props) {
      ...
      if (src && src.type) {
        ...
        this.isDefaultPrevented =
          src.defaultPrevented || (src.defaultPrevented === undefined && src.returnValue === false)
            ? returnTrue
            : returnFalse;
  ...
  (function () {
    ...
    var heatmap_render = {
      ...
      setHeatState: function (data, type, url, isFirst) {
        if (isFirst) {
          ...
          var obj = {
            ...
            "sa-request-url":
              sessionStorage && sessionStorage.getItem ? sessionStorage.getItem("sensors_heatmap_url") || "" : "",
          };
          try {
            var windowNameParam = {};
      ...
      setScrollMap: function (id, url) {
        ...
        if (typeof id === "string" && sd.para.web_url) {
          ...
          var suc = function (data) {
            ...
            var percent_tpl =
              '<div style="border-bottom: 1px dashed #4C4C4D;height:1px;width:100%;position: absolute;top:{{top}}px;"><span style="font-size:12px;position:absolute;padding:0 12px;top:-24px;height:26px;line-height: 26px;left:0;background:#000;color:#eee;border-radius: 2px;">{{percent}}</span></div>';
            for (var i in data.percent) {
              $(document.body).append(
                $(percent_tpl.replace("{{top}}", data.percent[i] - 2).replace("{{percent}}", i + "%"))
              );
            }

            var over_tpl =
              '<div style="z-index:99999;border-bottom: 1px solid #272727;height:1px;width:100%;position: absolute;top:{{top}}px;text-align:center;"><span style="font-size:12px;height:26px;line-height: 26px;background:#000;color:#eee;border-radius: 2px;left:50%;margin-left:-65px;position: absolute;top:-13px;padding: 0 5px;">{{percent}}的用户浏览到这里</span></div>';
            var over_ele = null;
      ...
      setToolbar: function (requrest_id, type, url) {
        ...
        div.setAttribute(
          "style",
          "height:50px !important;z-index:9999999;background:#272727;width:100%;position:fixed;top:0;left:0; font-size:14px;color:#EFF2F7;margin:0;clear: both;"
        );
        div.innerHTML =
        ...
        var toolbar_corner = $(
          '<div id="sa_sdk_heatmap_toolbar_corner" style="cursor:pointer;display:none;position: fixed;z-index:999999;top:0;right:10px;padding:3px 8px 0;background:#000;"></div>'
        );
        toolbar_corner.html(
        ...
        var filterFlyout =
          '<div id="sa_sdk_heatmap_filterFlyout" style="z-index: 999999; position: fixed; right: 2px; top: 51px; width:432.5px; height: 171px; background:rgba(255,255,255,1);box-shadow:0px 2px 9px 3px; rgba(10,10,10,0.39); border-radius:3px;"><form>';
        filterFlyout +=
          '<div style="height: 45px; text-align: center; line-height: 45px; font-size:14px; font-family:PingFangSC-Medium,PingFang SC; font-weight:500; color:rgba(32,45,61,1); border-bottom: 1px solid #E9F0F7;">';
        filterFlyout += "筛选展示元素";
        filterFlyout += "</div>";
        ...
        $(document.body).on(
          "click",
          "#sa_sdk_heatmap_filterFlyout .sa-sdk-heatmap-filterflyout-cancelbtn",
          function () {
        ...
        $(document.body).on(
          "click",
          "#sa_sdk_heatmap_filterFlyout .sa-sdk-heatmap-filterflyout-submitbtn",
          function () {
      ...
      showErrorInfo: function (error_type, error_msg) {
        ...
        div.setAttribute(
          "style",
          "background:#e55b41;border:none;border-radius:4px;color:#fff;font-size:18px;left:50%;margin-left:-300px;padding:12px;position: fixed;top:60px;text-align: center;width:600px;z-index:999999;"
        );
          ...
          if (obj.by_values[0]) {
            ...
            if (me.requestType === 3) {
              obj.data_user_link =
                ...
                "#heat_map_id=" +
                heat_map_id +
                "&detail=true&element_selector=" +
                encodeURIComponent(obj.by_values[0]) +
                "&page_url=" +
      ...
      renderHeatData: function (selector, data, key) {
        ...
        if (this.heatMode == 1) {
          ...
          wrap
            ...
            .attr("sa-click-area", this.heatData(data.data_click))
        ...
        if (this.heatMode == 1) {
          _.each(this.heatDataElement, function (ele) {
            ...
            if (tagName === "input" || tagName === "textarea" || tagName === "img" || tagName === "svg") {
              ...
              if (
                parent &&
                parent.ele.tagName.toLowerCase() === "span" &&
                !_.isUndefined($(parent.ele).attr("sa-click-area"))
              ) {
                $(ele.ele).unwrap();
              }
            } else {
      ...
      setCssStyle: function () {
        ...
        css +=
          "#sa_heat_float_right_box_content table td { color: #fff !important; font-size: 13px !important; background: #333 !important;}";
        css +=
          "#sa_sdk_heatmap_filterFlyout select {padding-left: 10px;width: 82px; height: 32px;background:rgba(255,255,255,1); border-radius:3px; border:1px solid rgba(211,220,230,1); margin-right: 10px; margin-left: 10px; outline: none;}";
        css += "#sa_sdk_heatmap_filterFlyout select:hover {border:1px solid rgba(4,203,148,1);}";
        css +=
          "#sa_sdk_heatmap_filterFlyout select:active, #sa_sdk_heatmap_filterFlyout select:focus {border:2px solid rgba(4,203,148,0.2);}";
        css +=
          "#sa_sdk_heatmap_filterFlyout input {outline:none;box-sizing: border-box; width:77px; height:32px; background:rgba(255,255,255,1); border-radius:3px; border:1px solid rgba(211,220,230,1); padding-left:10px; padding-right:10px;}";
        css += "#sa_sdk_heatmap_filterFlyout input:hover {border:1px solid rgba(4,203,148,1);}";
        css +=
          "#sa_sdk_heatmap_filterFlyout input:active #sa_sdk_heatmap_filterFlyout input:focus {border:2px solid rgba(4,203,148,0.2);}";
    ...
    var heatmap = {
            ...
            error: function (err) {
              ...
              if (err === "timeout") {
                ...
                if (_.URL(location.href).protocol === "https:" && _.URL(obj.url.jsonp).protocol === "http:") {
                  error({
                    error: "该页面协议为 https ，请使用 https 的growkonws后台查看热力图",
                  });
                } else {
                  error({
                    error: "<div>访问地址不通，请及时联系管理员</div>",
                  });
                }
              }
            },
      ...
      prepare: function (data, type, url) {
        ...
        if (
          ...
          sessionStorage.getItem("sensors_heatmap_url")
        ) {
